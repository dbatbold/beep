...
func OpenSoundDevice(device string) error {
	...
	status := C.AudioQueueNewOutput(
		...
		nil,
		(C.CFRunLoopRef)(unsafe.Pointer(nil)),
		(C.CFStringRef)(unsafe.Pointer(nil)),
		0,
		&audioQueue,
	)
...
func BatteryLevel() (int, error) {
	...
	if blob == (C.CFTypeRef)(unsafe.Pointer(nil)) {
		return 0, io.EOF
	}
	...
	description := C.IOPSGetPowerSourceDescription(blob, (C.CFTypeRef)(source))
	if description == (C.CFDictionaryRef)(unsafe.Pointer(nil)) {
		return 0, io.EOF
	}

	// Get current capacity
	// key := C.CFStringRef(C.CFStringCreateWithCString((C.CFAllocatorRef)(unsafe.Pointer(nil)), C.kIOPSCurrentCapacityKey, C.kCFStringEncodingUTF8))
